...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        ...
        # Check if the target node is a left or right child
        if level_pos % 2 == 0:
            path.append(state[level_pos + 1])
        else:
            path.append(state[level_pos - 1])

        # Construct the upper level by hashing pairs of data from the current level
        state = [hash_internal_node(state[i], state[i + 1]) for i in range(0, len(state), 2)]

        # Update the position of the target node
